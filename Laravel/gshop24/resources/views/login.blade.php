            ...
            <div class="tab-content">
                <div  id="home" class="modal-body mx-md-3 tab-pane active">
                    ...
                    <form id="login" action="{{url('/login')}}" method="post">
                        <div class="form-group {{ $errors->account->has('login-email') ? ' has-error' : '' }}">
                            <input type="text" name="login-email" class="form-control" placeholder="Nhập Email" value="" />
                        ...
                        <div class="form-group {{ $errors->account->has('login-password') ? ' has-error' : '' }}">
                            <input type="password" name="login-password" class="form-control" placeholder="Mật Khẩu" value="" />
                        ...
                        <div class="form-group">
                            <input type="submit" class="btnSubmit btn btn-info rounded-pill w-100" value="Đăng Nhập" />
                        </div>
                        <p class="font-small dark-grey-text text-right d-flex justify-content-center mb-3 pt-2"> hoặc Đăng nhập với:</p>
                        <div class="row my-3 d-flex justify-content-center">
                            <!--Facebook-->
                            <a href="{{ route('facebook.login') }}" class="btn btn-white btn-rounded mr-3 z-depth-1a waves-effect waves-light shadow rounded-pill w-25"><i class="text-info fab fa-facebook-f text-center"></i></a>
                            <!--Google +-->
                            <a href="{{ route('google.login') }}" class="btn btn-white btn-rounded z-depth-1a waves-effect waves-light shadow rounded-pill w-25"><i class="text-info fab fa-google"></i></a>
                          </div>
                        @csrf
                    </form>
                ...
                <div id="menu1" class="modal-body mx-md-3 tab-pane fade "><br>
                    ...
                    <form id="register" action="{{url('/register')}}" method="post">
                        <div class="form-group">
                            <input type="text" class="form-control" placeholder="Họ & Tên" value="" name="name"/>
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" placeholder="Nhập Email" value="" name="email"/>
                            @if($errors->account->has('email'))
                                <span class="text-danger">{{$errors->account->first('email')}}</span>
                            @endif
                        </div>
                        <div class="form-group">
                            <input type="password" class="form-control" placeholder="Mật Khẩu" value="" name="password" autocomplete="off"/>
                            @if($errors->account->has('password'))
                                <span class="text-danger">{{$errors->account->first('password')}}</span>
                            @endif
                        </div>
                        <div class="form-group">
                            <input type="submit" class="btnSubmit btn rounded-pill btn-info w-100" value="Tạo Tài Khoản"/>
                        </div>
                        @csrf
                    </form>
